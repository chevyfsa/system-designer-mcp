 ...
 * @param relationships - All relationships in the model
 * @param visited - Set of already visited entities (for recursion tracking)
...
function hasCircularInheritance(
  ...
  const inheritanceRels = relationships.filter(
    (rel) => rel.from === entityId && (rel.type === 'inheritance' || rel.type === 'implementation')
  );

  for (const rel of inheritanceRels) {
    if (hasCircularInheritance(rel.to, relationships, new Set(visited))) {
      return true;
    }
  }
...
/**
 * Get all entities that this entity inherits from (transitive closure)
 * @param entityId - The entity to check
 * @param relationships - All relationships in the model
 * @param allEntities - All entities in the model
 * @returns Array of ancestor entity names
 */
function getInheritanceChain(
  ...
  function traverse(currentId: string): void {
    ...
    const inheritanceRels = relationships.filter(
      (rel) =>
        rel.from === currentId && (rel.type === 'inheritance' || rel.type === 'implementation')
    );

    for (const rel of inheritanceRels) {
      const targetEntity = findEntityById(allEntities, rel.to);
      if (targetEntity && targetEntity.name !== '_Component') {
        ancestors.push(targetEntity.name);
        traverse(rel.to);
      }
    }
  }
  ...
  traverse(entityId);
  return Array.from(new Set(ancestors)); // Remove duplicates
}
